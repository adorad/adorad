...
static inline Token* chomp_if(Parser* parser, TokenKind tokenkind) {
    if((parser->curr_tok + 1) != null && (parser->curr_tok + 1)->kind is tokenkind)
...
func_no_attrs:;
    ...
    if(larrow is null and return_type_expr != null)
...
static AstNode* ast_parse_if_expr(Parser* parser) {
    ...
    if(lparen != null and rparen is null)
        ast_expected("closing `(`");
    if(lparen is null and rparen != null)
