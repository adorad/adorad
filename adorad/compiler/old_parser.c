...
static AstNode* ast_ast_ast_ast_ast_parse_match_expr(Parser* parser);
...
static AstNode* parse_binary_op_expr(Parser* parser, BinaryOpChain chain,
                                         AstNode* (*op_parser)(Parser*),
                                         AstNode* (*child_parser)(Parser*)
);
...
static AstNode* parse_func_prototype(Parser* parser) {
    ...
    Vec* params = parse_param_list(parser, parse_match_branch);
...
static AstNode* parse_loop_statement(Parser* parser) {
    ...
    // AstNode* loop_while_statement = parse_loop_while_statement(parser);
...
//      (BlockLabel)? block
static AstNode* parse_block_expr(Parser* parser) {
    Token* block_label = parse_block_label(parser);
    if(block_label != null) {
        AstNode* out = parse_block(parser);
...
static AstNode* parse_boolean_and_expr(Parser* parser) {
    return parse_binary_op_expr(
        parser,
        BinaryOpChainInfinity,
        parse_boolean_and_op,
        parse_boolean_or_expr
    );
}
